...
class Preprocessing:
    ...
    def clip_image(img, header):
        ...
        ovs_x_size = Configuration.OVS_X
        ovs_y_size = Configuration.OVS_Y

        # size of the taps
        chip_x_size = Configuration.TAP_X
        chip_y_size = Configuration.TAP_Y
    ...
    def correct_header(img, header, clean_file_name):
        ...
        working_directory_file = Configuration.OUTPUT_DIRECTORY + 'analysis/' + os.path.basename(clean_file_name) + '.temp'
    ...
    def mk_combined_bias_and_dark(image_overwrite):
        ...
        if ((os.path.isfile(Configuration.CALIBRATION_DIRECTORY + dark_name) == 0) or (os.path.isfile(Configuration.CALIBRATION_DIRECTORY + bias_name) == 0) or (image_overwrite == 'Y')):
            ...
            bias_bulk = None
            fits.writeto(Configuration.CALIBRATION_DIRECTORY + bias_name, bias, bias_header, overwrite=True)
            ...
            dark_header['EXPTIME'] = Configuration.DARK_EXP
            dark_header['BIAS_SUB'] = 'Y'
            dark = np.median(dark_bulk[0:zdx], axis=0)
            dark_bulk = None
            # write the image out to the master directory
            fits.writeto(Configuration.CALIBRATION_DIRECTORY + dark_name, dark, dark_header, overwrite=True)
    ...
    @staticmethod
    def mk_flat(flat_exp=Configuration.FLAT_EXP, dark_exp=Configuration.DARK_EXP):
        ...
        if not os.path.isfile(Configuration.CALIBRATION_DIRECTORY + 'flat' + Configuration.FILE_EXTENSION):

            # read in the bias and dark frames
            bias = fits.getdata(Configuration.CALIBRATION_DIRECTORY + 'bias' + Configuration.FILE_EXTENSION)
            dark = fits.getdata(Configuration.CALIBRATION_DIRECTORY + 'dark' + Configuration.FILE_EXTENSION)
            ...
            for kk in range(0, hold_bulk):
                ...
                flat_tmp_filename = Configuration.FLAT_DIRECTORY + str(tmp_num).zfill(2) + '_tmp_flat' + Configuration.FILE_EXTENSION
            ...
            # write the image out to the master directory
            fits.writeto(Configuration.CALIBRATION_DIRECTORY + 'flat' + Configuration.FILE_EXTENSION, nflat_image, flat_header, overwrite=True)

        else:
            nflat_image = fits.getdata(Configuration.CALIBRATION_DIRECTORY + 'flat' + Configuration.FILE_EXTENSION)

        return nflat_image

    @staticmethod
    def mk_nme(file, difference_image='N', bias_subtract='N', dark_subtract=
        'N', flat_divide='N', image_clip='N', sky_subtract='N',  plate_solve='N'):
    ...
    def sky_subtract(img, header, sky_write='N'):
        ...
        x_skip = Configuration.TAP_X
        y_skip = 2 * Configuration.OVS_Y + Configuration.OVS_X # 220
        y_full = Configuration.TAP_Y
