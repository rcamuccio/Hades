...
class Priority:
	...
	def field_generator(field_sep):
		...
		declination_limit = obs_params['declination_limit']
	...
	def generate_targets(detection_time=None):
		...
		lim_dec = df.dec > obs_params['declination_limit']
		new_df = df[lim_dec]
	...
	def night_field_selector(survey_fields, commissioning_fields, field_select='airmass'):
		...
		obs_params = Utils.config_observatory(Configuration.OBSERVATORY)
		longitude = obs_params['longitude']
		latitude = obs_params['latitude']
		elevation = obs_params['elevation']
		...
		f = open('/home/qu/Documents/hades-dev/analysis/nightly_observing_list_' + date_today + '.txt', 'w')
